...
public class NewReservationWindow {
    ...
    public static void openNewReservationWindow() 
    {
        ...
        newReservationFrame.setSize(550, 600);
        ...
        //JTextField for displaying insurance cost for each vehicle
        JTextField insurance_cost_field = CarRentalApp.createLabeledTextField(reservationPanel, "Insurance cost:");
        insurance_cost_field.setEditable(false);
        vehicle_type_box.addActionListener(e -> updateInsuranceCost(reservationPanel, insurance_cost_field, vehicle_type_box));
        vehicle_id_field.addFocusListener(new FocusListener() {
        	@Override
        	public void focusGained(FocusEvent e)
        	{
        	...
        	public void focusLost(FocusEvent e)
        	{
        		updateInsuranceCost(reservationPanel, insurance_cost_field, vehicle_type_box);
        	}
        });
        ...
        submitButton.addActionListener(e -> handleSubmission(reservationPanel, newReservationFrame, dateComboBoxes, 
        		durationComboBoxes, vehicle_type_box, someoneElseCheckBox, driverLicenseIdTextField,totalCostTextField,insurance_cost_field));
    ...
    private static void handleSubmission(JPanel reservationPanel, JFrame newReservationWindow, JComboBox<String>[] rentalDateBoxes, 
    		JComboBox<String>[] rentalDurationBoxes, JComboBox<String> vehicle_type_box, JCheckBox someoneElseCheckBox, JTextField driverLicenseIdTextField,JTextField TotalCostTextField,JTextField insurance_cost_field) 
    {
        ...
        String vehicle_id = CarRentalApp.getTextFromLabeledTextField(reservationPanel, "Vehicle id:");
        ...
        float totalCost = Float.parseFloat(TotalCostTextField.getText());
        float insuranceCost = Float.parseFloat(insurance_cost_field.getText());
        ...
        if (mainBack.user != null) {
            try {            	
                ...
                Matcher licenseMatcher = licensePattern.matcher(vehicle_id);
                Matcher integerMatcher = integerPattern.matcher(vehicle_id);

                // Check if the input matches the pattern
                if (!licenseMatcher.matches() && (vehicle_type.equals("Car") || vehicle_type.equals("Motorcycle"))) 
                {
                	JOptionPane.showMessageDialog(newReservationWindow, "Wrong vehicle id format", "Reservation Error",
                            JOptionPane.ERROR_MESSAGE);
        			return;
                }else {
                	calendar.setTime(mainBack.user.getBirthdate());
                	birthdayYear=calendar.get(Calendar.YEAR);
                	if(currentYear-birthdayYear<18) {
                		JOptionPane.showMessageDialog(newReservationWindow, "The driver is under the age of 18.", "Reservation Error",
                                JOptionPane.ERROR_MESSAGE);
            			return;
                	}
                }
                if (!integerMatcher.matches() && (vehicle_type.equals("Bicycle") || vehicle_type.equals("Scooter")))	// Check for bicycle and scooter
                {
                	JOptionPane.showMessageDialog(newReservationWindow, "Wrong vehicle id format, not a positive integer.", "Reservation Error",
                            JOptionPane.ERROR_MESSAGE);
        			return;
                }else {
                	calendar.setTime(mainBack.user.getBirthdate());
                	...
                	if(currentYear-birthdayYear<16) {
                		JOptionPane.showMessageDialog(newReservationWindow, "The driver is under the age of 16.", "Reservation Error",
                                JOptionPane.ERROR_MESSAGE);
            			return;
                	}
                }
                if(payment_amount<totalCost) {
                	JOptionPane.showMessageDialog(newReservationWindow, "The vehicle's cost has not been covered.", "Reservation Error",
                            JOptionPane.ERROR_MESSAGE);
        			return;
                }
                if(payment_amount-totalCost > insuranceCost ) {
                	JOptionPane.showMessageDialog(newReservationWindow, "The insurance cost has been covered.", "Reservation State",
                ...
                JOptionPane.showMessageDialog(newReservationWindow, "Reservation complete!", "Reservation success",
                        JOptionPane.INFORMATION_MESSAGE);
    ...
    private static boolean updateRentCounter(JFrame newReservationWindow, String vehicle_type, String vehicle_id) throws ClassNotFoundException, SQLException
    {
    ...
    private static void updateInsuranceCost(JPanel reservationPanel, JTextField insurance_cost_field,JComboBox<String> vehicle_type_box) 
    {
    	String vehicle_id = CarRentalApp.getTextFromLabeledTextField(reservationPanel, "Vehicle id:"); // Will parse to integer depending on vehicle type
		String vehicle_type = (String) vehicle_type_box.getSelectedItem();
		float insurance_cost = 1;
    	...
    	try {
            ...
            if (!licenseMatcher.matches() && (vehicle_type.equals("Car") || vehicle_type.equals("Motorcycle"))) 
            {
            	JOptionPane.showMessageDialog(reservationPanel, "Wrong vehicle id format", "Reservation Error",
                        JOptionPane.ERROR_MESSAGE);
    			return;
            }
            else if (!integerMatcher.matches() && (vehicle_type.equals("Bicycle") || vehicle_type.equals("Scooter")))	// Check for bicycle and scooter
            {
            	JOptionPane.showMessageDialog(reservationPanel, "Wrong vehicle id format, not a positive integer.", "Reservation Error",
                        JOptionPane.ERROR_MESSAGE);
    			return;
            }
			...
			insurance_cost_field.setText(String.valueOf(insurance_cost));
    	}
